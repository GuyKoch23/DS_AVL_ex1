...
class AVLTree(object):
	...
	# CR: function to confusing, not possible to understand your algorithm due to no comments or structure.
	def join_to_left(self, tree, key, val):
		...
		temp = connector
		temp_son = None
		# CR: this part does not exist in join_to_rigt.
		...
		node = connector
		temp = None
		# CR: I think I understand what is going on here, but this entire function is very hard to read. It looks find, but honestly, the CR
		#     I did here is soso, just because the lack of comments makes this very hard to read.
		while node != None:
			bf = self.calculate_BF(node)
			if abs(bf) >= 2:
				self.rotate(node, bf)
	...
	# CR: I am sure you could have avoided the code duplication of this function. But no big deal for this exercise.
	...
	# CR: This is find if if it simplifies the exercise for you. I am not sure it is truly needed.
	def join_tree_with_empty(self, tree, key, val):
		...
		if abs(bf) >= 2:
			...
			# CR: Wouldn'nt you potentially need to update heights here?
	...
	# CR: Comments in this function would do wonders for the reader - but I think this works well and the algorithm looks fine.
	def split(self, node):
